...
describe("Fluxos de Integracao", () => {
  it("fluxo completo de geracao", () => {
    const input = {
      ...
      tone: "engracado" as Tone,
      topic: "Bolo de caneca",
      transcript: "Hoje vou ensinar...",
    };
    ...
    const aiResponse = {
      ...
      captions: ["C1", "C2", "C3"],
      emojis: ["🎂", "⏱️", "🔥", "😋", "✨", "🧁"],
      hashtags: ["#bolo", "#receita", "#doces", "#tiktokfood"],
    };
  ...
  it("limite gratuito atingido", () => {
    expect(
      { plan: "free" as const, count: 5, limit: FREE_DAILY_LIMIT }.count < FREE_DAILY_LIMIT,
    ).toBe(false);
  });

  it("plano pro permite uso alto", () => {
    expect(
      { plan: "pro" as const, count: 100, limit: PRO_DAILY_LIMIT }.count < PRO_DAILY_LIMIT,
    ).toBe(true);
  });
  ...
  it("rejeita topicos curtos", () => {
    expect("ab".length).toBeLessThan(3);
  });
  it("aceita topicos validos", () => {
    const t = "Como crescer no Instagram";
    expect(t.length).toBeGreaterThanOrEqual(3);
    expect(t.length).toBeLessThanOrEqual(400);
  });
  it("rejeita transcricoes longas", () => {
    expect("a".repeat(5001).length).toBeGreaterThan(5000);
  });
  it("webhook usa HTTPS", () => {
    expect("https://app.com/api/public/stripe-webhook".startsWith("https://")).toBe(true);
  });
});
