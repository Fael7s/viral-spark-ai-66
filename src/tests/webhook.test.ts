...
describe("Webhook", () => {
  ...
  async function handle(req: Request): Promise<Response> {
    ...
    let ev: Stripe.Event;
    try {
      ev = await stripe.webhooks.constructEventAsync(body, sig, secret);
    } catch {
        ...
        await db.from("subscriptions").upsert(
          {
            ...
            current_period_end: sub.current_period_end
              ...
              : null,
            last_stripe_event_created: new Date(ev.created * 1000).toISOString(),
  ...
  it("cria assinatura pro apos checkout", async () => {
    ...
    expect(mockDb.upsert).toHaveBeenCalledWith(
      expect.objectContaining({ user_id: "u", plan: "pro" }),
      expect.anything(),
    );
  });
  ...
  it("ignora sem subscription", async () => {
    ...
    const ev = {
      ...
      created: 1,
      data: {
        object: { metadata: {}, client_reference_id: null, subscription: null, customer: null },
      },
    } as unknown as Stripe.Event;
    mockStripe.webhooks.constructEventAsync.mockResolvedValueOnce(ev);
    const res = await handle(
  ...
  it("processa quando nao ha registro anterior", () => {
    const isNewer = (t: number, last: string | null) =>
      !last || t * 1000 > new Date(last).getTime();
