...
describe("Regras de Negocio", () => {
  ...
  it("3 plataformas", () => {
    const p = PLATFORMS.map((x) => x.value);
    expect(p).toEqual(["tiktok", "reels", "shorts"]);
  });
  it("plataformas tem label e hint", () =>
    PLATFORMS.forEach((p) => {
      expect(p.label.length).toBeGreaterThan(0);
      expect(p.hint.length).toBeGreaterThan(0);
    }));
  it("6 tons", () => expect(TONES).toHaveLength(6));
  it("tons pro: provocativo e luxo", () => {
    const pro = TONES.filter((t) => t.pro).map((t) => t.value);
    expect(pro).toContain("provocativo");
    expect(pro).toContain("luxo");
    expect(pro).toHaveLength(2);
  });
  it("4 tons gratuitos", () => expect(TONES.filter((t) => !t.pro)).toHaveLength(4));
  it("segredos sem VITE_", () => {
    const env = "STRIPE_SECRET_KEY=\nSTRIPE_WEBHOOK_SECRET=";
    expect(env).not.toContain("VITE_STRIPE");
  });
